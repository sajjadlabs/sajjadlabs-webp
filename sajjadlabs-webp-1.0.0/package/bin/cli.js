...
async function start() {
    ...
    const mode = await p.select({
        ...
        options: [
            ...
            { name: 'manual', value: 'manual', label: '🔧 Custom Mode', hint: 'Manual Settings' }
        ],
    });
    ...
    if (mode === 'manual') {
        const qualityChoice = await p.select({
            ...
            options: [
                ...
                { value: 'custom', label: '⌨️  Custom Number', hint: 'Enter a specific value' }
            ],
        });
...
async function optimizeImages(inputPath, options, spinner) {
    ...
    const outputDir = path.join(path.dirname(absolutePath), path.basename(absolutePath) + '_optimized');
    ...
    const imageFiles = files.filter(file =>
        ['.jpg', '.jpeg', '.png', '.tiff', '.tif', '.webp'].includes(path.extname(file).toLowerCase())
    );
    ...
    const savedPercentage = totalOriginalSize > 0 ? ((savedSpace / totalOriginalSize) * 100).toFixed(1) : 0;

    spinner.stop(`✨ Successfully processed ${successCount} out of ${imageFiles.length} images!`);
